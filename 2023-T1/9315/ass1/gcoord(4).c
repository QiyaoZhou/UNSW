...
void check_gcoord_in(char *location){	
	...
	if(*location_name == ' '){		
		...
		ereport(ERROR,
						...
						errmsg("0invalid input syntax for type %s: \"%s\"",
						"GeoCoord", location)));
	...
	if ( *(position - 1) == ' ') {
            ...
            ereport(ERROR,
						(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
						errmsg("1invalid input syntax for type %s: \"%s\"",
						"GeoCoord", location)));
    }
    ...
    if (third == NULL) {
        ...
        if (third == NULL) {
            ...
            ereport(ERROR,
						(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
						errmsg("2invalid input syntax for type %s: \"%s\"",
						"GeoCoord", location)));
        }
    }
	...
	if(strchr(third,'W')!=NULL || strchr(third,'E')){
		...
		*longitude ='\0';
		...
		if (strchr(latitude,'W')||strchr(latitude,'E')){
			...
			ereport(ERROR,
						(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
						errmsg("3invalid input syntax for type %s: \"%s\"",
						"GeoCoord", location)));
			...
			if (strncmp(latitude+strlen(latitude)- 3, DEGREE, 2) != 0 || strncmp(longitude+strlen(longitude)- 3, DEGREE, 2) != 0 ) {
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
						errmsg("4invalid input syntax for type %s: \"%s\"",
						"GeoCoord", location)));
			}else{
				...
				*(longitude+strlen(longitude)- 3) ='\0';
				if(atoi(latitude)<0 || fabs(atoi(latitude)-(int)atoi(latitude))>=1e-8 || atoi(latitude)>90 || atoi(longitude)<0 || fabs(atoi(longitude)-(int)atoi(longitude))>=1e-8 || atoi(longitude)>180){
					...
					ereport(ERROR,
						(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
						errmsg("5invalid input syntax for type %s: \"%s\"",
						"GeoCoord", location)));
				//有度数则取出数字部分
				}else{
					...
					if(count>1){
						ereport(ERROR,
					  				(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
					  				errmsg("6invalid input syntax for type %s: \"%s\"",
					  				"GeoCoord", location)));
					}
		...
		*latitude ='\0';
		...
		if(strchr(longitude,'N')||strchr(longitude,'S')){
			ereport(ERROR,
						(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
						errmsg("7invalid input syntax for type %s: \"%s\"",
						"GeoCoord", location)));
			...
			if (strncmp(latitude+strlen(latitude)- 3, DEGREE, 2) != 0 || strncmp(longitude+strlen(longitude)- 3, DEGREE, 2) != 0 ) {
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
						errmsg("8invalid input syntax for type %s: \"%s\"",
						"GeoCoord", location)));
			}else{
				*(latitude+strlen(latitude)- 3) ='\0';
				printf("%s\n",latitude);
				*(longitude+strlen(longitude)- 3) ='\0';
				...
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
						errmsg("9invalid input syntax for type %s: \"%s\"",
						"GeoCoord", location)));
        ...
        ereport(ERROR,
				(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
					errmsg("10invalid input syntax for type %s: \"%s\"",
					"GeoCoord", location)));
    }
    ...
    for (int i = 0; i < strlen(location_name); i++){
        ...
        if(isalpha(location_name[i])||location_name[i] ==' '){
            ...
            ereport(ERROR,
				(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
					errmsg("11invalid input syntax for type %s: \"%s\"",
					"GeoCoord", location)));
        }
    }
}
