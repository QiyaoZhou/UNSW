...
int com_long(GeoCoord * a, GeoCoord * b);
int com_long(GeoCoord * a, GeoCoord * b){
	GeoCoord_Info g1 = string_to_struct(a->gcoord);
	GeoCoord_Info g2 = string_to_struct(b->gcoord);	

	if(atof(g1.longitude)/15 - atof(g2.longitude)/15>=-1 && atof(g1.longitude)/15 - atof(g2.longitude)/15 <=1){
			...
			if (strncmp(latitude+strlen(latitude)- 3, DEGREE, 2) != 0 || strncmp(longitude+strlen(longitude)- 3, DEGREE, 2) != 0 ) {
				...
				*(longitude+strlen(longitude)- 3) ='\0';
				...
				//有度数则取出数字部分
					...
					count = 0;
					for (int i = 0; i < strlen(latitude); i++) {
        				if (*(latitude+1) == '.') {
            				count++;
            				if (count > 1) {
                				ereport(ERROR,
...
char *return_dms_format(char *CoordValue);
char *return_dms_format(char *CoordValue){
    char *word = strdup(CoordValue);
    float full_num;
    int D;
    int M;
    int S;
...
        D = atoi(word);
        
        M = round((atof(word) - D) * 60);
        elog(NOTICE,"=====%d\n",3600*(atof(word) - D));
        
        S = 3600*(atof(word) - D)-60*M;
        
        elog(NOTICE,"%d\n",D);
        elog(NOTICE,"%d\n",M);
        elog(NOTICE,"%d\n",S);
...
gcoord_out(PG_FUNCTION_ARGS)
{
	...
	char	   *result;
	GeoCoord_Info G1 = string_to_struct(geoCoord->gcoord);
	result = psprintf("%s,%s%s%s,%s%s%s",G1.location,G1.latitude,DEGREE,G1.lat_dir,G1.longitude,DEGREE,G1.long_dir);
...
convert2dms(PG_FUNCTION_ARGS)
{
	...
	text *result_dms;
	if(G1.first ==2){
		char *result = psprintf("%s,%s%s,%s%s",G1.location,return_dms_format(G1.latitude),G1.lat_dir,return_dms_format(G1.longitude),G1.long_dir);
