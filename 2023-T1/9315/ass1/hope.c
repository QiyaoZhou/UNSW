...
convert2dms(PG_FUNCTION_ARGS)
{
	...
	}else if(G1.first ==1&&G1.mark ==2){
		char *result = psprintf("%s,%s%s,%s%s",G1.location,return_dms_format(G1.latitude),G1.lat_dir,return_dms_format(G1.longitude),G1.long_dir);
		result_dms = cstring_to_text(result);
	}else if(G1.first ==2&&G1.mark ==1){
		char *result = psprintf("%s,%s%s %s%s",G1.location,return_dms_format(G1.longitude),G1.long_dir,return_dms_format(G1.latitude),G1.lat_dir);
		result_dms = cstring_to_text(result);
	}else{
		char *result = psprintf("%s,%s%s,%s%s",G1.location,return_dms_format(G1.longitude),G1.long_dir,return_dms_format(G1.latitude),G1.lat_dir);
		result_dms = cstring_to_text(result);
