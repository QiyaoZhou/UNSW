...
// advance clock hand
    ...
    while (slot == NONE) {
        int c = pool->clock;
        if (pool->bufs[c].pin == 0) count_zero += 1;
        count_scanned ++;
        if (pool->bufs[c].pin == 0 && pool->bufs[c].pop == 0) {
            slot = c;
            break;
        }
        if (count_scanned >= pool->nbufs && count_zero == 0) return NONE;
        if (pool->bufs[c].pop > 0)  pool->bufs[c].pop -= 1;
        pool->clock = (1 + pool->clock)%pool->nbufs;
        
    }
    
    if (slot == NONE) return NONE;
    if (pool->bufs[slot].dirty > 0) pool->nwrites ++;
    pool->clock = (1 + pool->clock)%pool->nbufs;
    return slot;
}
