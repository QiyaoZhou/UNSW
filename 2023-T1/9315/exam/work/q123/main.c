...
void run(char* ra_path, char* log_path){
    ...
    while(fgets(line,100,query_fp)){
        ...
        // process projection (with distinct)
        if(line[0] == 'p'){
            char ra[20];
            UINT idx = 0;
            char table_name[50];
            ...
            // ra is "projection"
            sscanf(line,"%s %u %s",ra,&idx,table_name);

            reset_IO();

            _Table* result = proj(idx,table_name);
        ...
        if(line[0] == 'i'){
            ...
            token = strtok(NULL,s);
            UINT columns;
            sscanf(token,"%u",&columns);

            INT* tuple = (INT*)malloc(columns*sizeof(INT));

            for (UINT i = 0; i < columns; ++i){
                token = strtok(NULL,s);
                sscanf(token,"%d",&tuple[i]);
            }

            char* table_name = strtok(NULL,s);


            reset_IO();

            ins(tuple,table_name);
            
            Conf* cf = get_conf();
            // write the result to log file
            fprintf(log_fp,"\n######\n");
            fprintf(log_fp,"%u %u\n\n",cf->read_io,cf->write_io);
        ...
        // calculate io for n-way sort merge
        if(line[0] == 'c'){
            char ra[20];
            UINT nattrs = 0;
            UINT ntupples = 0;
            ...
            // ra is "calculate sort io"

            sscanf(line,"%s %u %u %u %u",ra,&nattrs,&ntupples,&page_size,&nslots);

            UINT time = calculate_sort_time(nattrs,ntupples,page_size,nslots);
...
void logT(_Table* t, FILE* log_fp){
    ...
    Conf* cf = get_conf();
    fprintf(log_fp,"%u %u %u %u\n\n",t->nattrs,t->ntuples,cf->read_io,cf->write_io);
