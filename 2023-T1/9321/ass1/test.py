...
def question_4(df1):
    ...
    #################################################
    # Your code goes here ...
    process = df1.groupby(['Country', 'Month']).agg(
        Count=pd.NamedAgg(column='AustralianPort', aggfunc='count')
    ).reset_index()
    df4 = process.groupby(['Country']).agg(
        Unique_ForeignPort_Count=pd.NamedAgg(column='Count', aggfunc='sum')
...
def question_5(seats):
    ...
    #################################################
    # Your code goes here ...
    def check2(input,model):
        if (input == 'I' and model == 'D') or (input == 'O' and model == 'S'):
            res = 'Australian_City'
        elif (input == 'I' and model == 'S') or (input == 'O' and model == 'D'):
            res = 'International_City'
        return res

    df5 = pd.read_csv(seats)
    df5['Source_City'] = df5.apply(lambda x: check2(x['In_Out'], 'S'), axis=1)
    df5['Destination_City'] = df5.apply(lambda x: check2(x['In_Out'], 'D'), axis=1)
    #################################################
