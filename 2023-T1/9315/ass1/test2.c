...
int convert(GeoCoord coordinate){
    char *result = coordinate->loc;
    strcat(result, ",");
    int deg1 = (int)floor(coordinate->lat);
    int min1 = (int)floor((coordinate->lat-deg1)*60);
    int sec1 = (int)floor(((coordinate->lat-deg1)*60-min1)*60);

    int deg2 = (int)floor(coordinate->lon);
    int min2 = (int)floor((coordinate->lon-deg2)*60);
    int sec2 = (int)floor(((coordinate->lon-deg2)*60-min2)*60);
    char deg_lat[]="";
    itoa(deg1,deg_lat,10);
    strcat(result, deg_lat);
    strcat(result, "°");
    char min_lat[]="";
    itoa(min1,min_lat,10);
    strcat(result, min_lat);
    strcat(result, "\'");
    char sec_lat[]="";
    itoa(sec1,sec_lat,10);
    strcat(result, sec_lat);
    if(coordinate->lat==1){
        strcat(result, "\"S");
    }else{
        strcat(result, "\"N");
    ...
    char deg_lon[]="";
    itoa(deg2,deg_lon,10);
    strcat(result, deg_lon);
    strcat(result, "°");
    char min_lon[]="";
    itoa(min2,min_lon,10);
    strcat(result, min_lon);
    strcat(result, "\'");
    char sec_lon[]="";
    itoa(sec2,sec_lon,10);
    strcat(result, sec_lon);
