...
int locationname_check(char *str){
    ...
    if(strcmp(str + strlen(str) - 4, " °N")==0||strcmp(str + strlen(str) - 4, " °S")==0||strcmp(str + strlen(str) - 4, " °W")==0||strcmp(str + strlen(str) - 4, " °E")==0){
        return 0;
    }
...
not_same_time_zone(PG_FUNCTION_ARGS){
    ...
    GeoCoord_Split coordinate1 = restore(a->gcoord);
    GeoCoord_Split coordinate2 = restore(b->gcoord);
    if(coordinate1.lon_m==coordinate2.lon_m&&(int)floor(coordinate1.lon/15)==(int)floor(coordinate2.lon/15)){
        result = 0;
    }else{
        result = 1;
    }
